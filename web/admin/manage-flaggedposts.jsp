...
<html lang="en">
    <head>
        ...
        <script>
            ...
            function populateUnFlagPostModal(postID){
                postList.forEach(function(post){
                    if(post.postId == postID){
                        $("#usernameUnflagLabel").text(post.firstName + " " + post.lastName);
                        $("#unflag_firstName").text(post.firstName);
                        $("#unflag_lastName").text(post.lastName);
                        $("#unflag_postDate").text(post.date);
                        $("#unflagId").val(post.postId);
                    }
                })
            }
                        ...
                        <div id="post-${post.postId}" class="postWrapper row-fluid">
                            ...
                            <div class="span2 postSideBlock">
                                ...
                                <div class="unflagBar">
                                    ...
                                    <a href="#unflagPostModal" onclick="populateUnFlagPostModal(${post.postId})" data-toggle="modal" class="float_r flagPost flagInappropriateBtn"><i class="icon-flag"></i> <span class="txt">Remove Flag</span></a>
...
<div id="unflagPostModal" class="modal hide fade">
    <div id="myModal" class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">x</button>
        <h3>Unflagging of <span id="usernameUnflagLabel"></span>'s post</h3>
    </div>
    <div class="modal-body">
        <stripes:form class="form-horizontal" beanclass="com.lin.resident.ManagePostBean" focus=""> 
            You are now Unflagging <b><span id="unflag_firstName"></span> 
                <span id="unflag_lastName"></span>'s</b> post on the <b>
                <span id="unflag_postDate"></span>
            </b>. Are you sure?
        </div>
        <div class="modal-footer">
            <a data-dismiss="modal" class="btn">Close</a>
            <stripes:hidden id="unflagId" name="postId"/>
            <input type="submit" name="adminUnFlagPostInappropriate" value="Confirm Unflag" class="btn btn-primary"/>
        </div>
    </stripes:form>
</div>
